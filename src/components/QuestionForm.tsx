...
export default function QuestionForm({
  ...
  const [formData, setFormData] = useState({
    ...
    attachmentId: null as File | null,
  });
  ...
  const create = async () => {
    if (!formData.attachmentId) throw new Error("Please upload an attachment");
  ...
  const update = async () => {
    if (!question) throw new Error("Please select a question");

    const attachmentId = await (async () => {
      if (formData.attachmentId) {
        if (!formData.attachmentId) return question?.attachmentId as string;

        await storage.deleteFile(
          questionAttachmentCollection,
          question?.attachmentId as string
        );
