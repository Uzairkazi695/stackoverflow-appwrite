...
export default async function Page({
  ...
  if (searchParams.voteStatus)
    queries.push(Query.equal("voteStatus", searchParams.voteStatus));

  const votes = await databases.listDocuments(db, voteCollection, queries);
  ...
  votes.documents = await Promise.all(
    votes.documents.map(async (vote) => {
      ...
      const questionOfTypeAnswer = answer.questionId
  ...
  return (
    <div className="px-4">
      ...
      <Pagination total={votes.total} limit={20} />
    </div>
  );
}
