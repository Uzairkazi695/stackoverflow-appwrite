...
const Notification = async ({ user }: { user: Models.User<UserPrefs> }) => {
  const authorPrefs = await users.getPrefs(user.$id);
  return (
    <figure
      className={cn(
...
export default async function TopContributors() {
  ...
  return (
    <div className="relative flex max-h-[400px] min-h-[400px] w-full max-w-[32rem] flex-col overflow-hidden rounded-lg bg-white/10 p-6 shadow-lg ml-5">
      <AnimatedList>
